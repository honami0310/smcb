...
if ($personalid !== null) {
  $stmt_keireki = $pdo->prepare("SELECT * FROM gs_$personalid ");
...
// 3-3-4. SMBスコア(=回答数×評価+質問数/2 )を出す
$view_smb_score = "";
$smb_score = number_format($a_count * $star_ave + $q_count / 2,1);
...
if ($status_mysmb_a == false) {
  ...
  while ($res_mysmb_a = $stmt_mysmb_a->fetch(PDO::FETCH_ASSOC)) {
    ...
    $view_mysmb_a .= '<td>' . h($res_mysmb_a['reviewstar']) . '</td>';
    ...
    if ($res_mysmb_a["onoroff"] == '知恵袋'){
      $view_mysmb_a .= '<td><a href="chie_chat.php? threadid=' . h($res_mysmb_a["id"]) . '">[知恵袋を見る]</a></td>';
    }
      else {
        $view .= '<td></td>';;
      };
...
if ($status_mysmb_q == false) {
  ...
  while ($res_mysmb_q = $stmt_mysmb_q->fetch(PDO::FETCH_ASSOC)) {
    ...
    $view_mysmb_q .= '<td>' . h($res_mysmb_q['reviewstar']) . '</td>';
...
<body id="main">
  ...
  <h3 class="obi_left">SMB実績</h3>
  <table id="smb_left">
    <tr><td>回答したSMCB数：</td><td><?= $a_count ?></td><td rowspan="4" id="starRank"></td></tr>
    <tr><td>質問したSMCB数：</td><td><?= $q_count ?></td></tr>
    <tr><td>SMCB評価：</td><td><?= $star_ave ?></td></tr>
...
if (smb_score>=200){star_rank ="スーパーエリート";}
else if(200>smb_score && smb_score>=100){star_rank ="エリート";}
else if(100>smb_score && smb_score>=50){star_rank ="アチーバー";}
else if(50>smb_score && smb_score>=20){star_rank ="リーディングメンター";}
